...
def getSignflipRatio(hBkg_pimu, hSig, hData, mlow, mhigh, mlow2, mhigh2, mlow3, mhigh3, key, start, stop,findcut = None, cut = None):
  ...
  #also prepare exponential for mass correction
  import pdb
  tau          = ROOT.RooRealVar    ("tau", "tau", -0.01, -1.0, 0.0);
  pdf_exp      = ROOT.RooExponential("exp", "exp", dsMass, tau)
  ...
  pdf_exp_ext   = ROOT.RooExtendPdf("pdf_exp_ext"     , "pdf_exp_ext"     , pdf_exp      , nData, "complete") 
  ...
  exp_result = pdf_exp_ext  .fitTo(hData_Roo, ROOT.RooFit.Range("complete"), ROOT.RooFit.SumW2Error(True))
  ...
  pdf_exp  .plotOn(frame,ROOT.RooFit.LineColor(ROOT.kBlue ),            ROOT.RooFit.Name("pdf_exp"), ROOT.RooFit.FillColor(ROOT.kYellow  - 7), ROOT.RooFit.DrawOption("L") )
  ...
  #text.AddText(" #pimu frac. = "  + f"{p_scale} ")
  text.AddText(" bkg. frac  = "  + f"{b_scale} ")
...
def fitAnotherVar   (hBkg_kk, hBkg_pimu, hSig, hData, mlow, mhigh, mlow2, mhigh2, mlow3, mhigh3, key, start, stop):
  ...
  #bkg_scale   = ROOT.RooRealVar   ("bkg_scale",     "bkg_scale",  0.7643)
  sig_scale   = ROOT.RooFormulaVar("sig_scale",     "1.0 - bkg_scale", ROOT.RooArgList(bkg_scale))
  ...
  #redefine data with Ptr

  #do a sideband fit, possible when using unconstrained data!
  sigma = 0.009

  #signal region
  mlow   = dsMass_ - 3*sigma
  mhigh  = dsMass_ + 3*sigma

  #sideband start
  mlow2  = dsMass_ - 3*sigma
  mhigh2 = dsMass_ + 3*sigma

  #sideband stops
  mlow3  = mlow2  - 2*sigma
  mhigh3 = mhigh2 + 2*sigma 

  # adjust histo style
  hData.GetXaxis().SetTitle(r"D_{s} mass [GeV]")
  hData.GetXaxis().SetTitleOffset(1.3)
  hData.GetYaxis().SetTitleSize(0.8*hData.GetYaxis().GetTitleSize())
  hData.GetXaxis().SetTitleSize(0.8*hData.GetXaxis().GetTitleSize())
  ...
  hData_Roo      = ROOT.RooDataHist("hData_Roo"     , "hData_Roo"    , ROOT.RooArgList(combMass), hData    )
  
  #add some more observable ranges
  ...
  combMass.setRange("compelte" ,start ,stop)
  ...
  #print(binEdges) 
  # Define ranges for every bin, this does not work within a for loop -.-
  ...
  nDataVar       = ROOT.RooRealVar(    "nDataVar",       "nDataVar",             n0_Data) 
  #parameters for exp
  ...
  #extend the gauss!
  expSig        = ROOT.RooRealVar(      "expSig","expSig", n0_Data * 0.07, 0, n0_Data) # nr of signals
  ...
  resultEx = modelEx.fitTo(hData_Roo)     
  ...
  frame_comb.GetYaxis().SetRangeUser(0,hData.GetMaximum()*1.4)
  ...
  hData_Roo.plotOn(frame_comb,ROOT.RooFit.Name("data"))#,ROOT.RooFit.Range("complete")
  frame_comb.Print("v")
  ...
  S = n0_Data*modelEx.createIntegral(combMass,combMass,"signal").getVal() - nBkg*expEx.createIntegral(combMass,combMass,"signal").getVal()
  ...
  #for i in range(bins):
  #	#fill hgauss
  #	event = hpull.Eval(hSig.GetBinCenter(i+1))
  #	a = hgauss.Fill(event)
  #hgauss.GetYaxis().SetTickSize(0)
  #hgauss.GetXaxis().SetTickSize(0.1)
  #hgauss.GetXaxis().SetNdivisions(ratio_bins)
...
def getSignflipRatioIncl(hBkg, hSig, hData, mlow, mhigh, mlow2, mhigh2, mlow3, mhigh3, key, start, stop):
  ...
  #perform the fit
  #result = pdf_total.fitTo(hData_Roo, ROOT.RooFit.Range("dsMassRange"), ROOT.RooFit.SumW2Error(True))
  result = pdf_total_ext.fitTo(hData_Roo, ROOT.RooFit.Range("complete"), ROOT.RooFit.SumW2Error(1))
...
def getSignflipRatioTest(hBkg, hSig, hData, mlow, mhigh, mlow2, mhigh2, mlow3, mhigh3, key, start, stop, findcut = None, cut = None):
