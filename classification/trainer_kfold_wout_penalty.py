...
if args.prod == "24":
  ...
  if trigger == "mu7":
    trigger_data = " && ( found_photon>0) &&  (mu7_ip4)" #on data we select on the trigger
    trigger_mc   = " && ( found_photon>0) &&  (mu7_ip4) && (event % 20 < 10) " #split by evt nr 
  else:
    trigger_data = " && ( found_photon>0) &&  ((mu9_ip6) && !(mu7_ip4)) && (mu_pt > 9.0) "
    trigger_mc   = " && ( found_photon>0) &&  (mu9_ip6) && (event % 20 >= 10) && (mu_pt > 9.0)" #no prescale on MC thus mu9 is enough
...
class Trainer(object):
  ...
  def train(self, xx_folds, y_folds, w_folds, weight, class_label):
    '''
    ...
    for n in range(self.nfolds):
      ...
      self.plotMetric(history, "loss"      ,"Training Loss"                                                     , "Loss"   , fold = n)
      self.plotMetric(history, "acc"       ,"Training Accuracy"                                                 , "Acc."   , fold = n)
      self.plotMetric(history, "val_loss"  ,"Validation Loss"                                                   , "Loss"   , fold = n)
      self.plotMetric(history, "val_acc"   ,"Validation Accuracy"                                               , "Acc."   , fold = n)
  ...
  def plotMetric(self, history, history_key, title, ylabel, fold = None):
    ...
    if fold is None:
      ...
      max_epochs = max(len(history[n].history[history_key]) for n in range(self.nfolds))
  
      for n in range(self.nfolds):
  
        toPlot = history[n].history[history_key]
  ...
  def process(self):
    ...
    # do the training
    print('\n========> training...') 
    model, history, xx_train, y_train, xx_val, y_val = self.train(xx_folds, y_folds, w_folds, weight, class_label)
