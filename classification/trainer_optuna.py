...
def define_model(trial):
  ...
  regu_rate     = trial.suggest_float('regu_rate', 0.0, 0.5)
       
  for i in range(n_layers):
    num_nodes     = trial.suggest_int(f'num_nodes_{i}', 4, 128)
    model.add(tf.keras.layers.Dense(num_nodes, activation='relu', kernel_regularizer=regularizers.l2(regu_rate))) 
 ...
 def create_optimizer(trial):
   ...
   if optimizer_selected == "Adam": kwargs["learning_rate"] = trial.suggest_float("adam_learning_rate", 1e-5, 1e-1, log=True)
   else optimizer_selected == "SGD":  kwargs["learning_rate"] = trial.suggest_float("sgd_opt_learning_rate", 1e-5, 1e-1, log=True)
