...
class torchModel(nn.Module):
    ...
    def __init__(self, dimIn, dimOut=6):
        ...
        self.out = nn.Linear(64   , dimOut)

    #define activation
    def forward(self, x):
        x = F.relu(self.fc1(x))   # swish is silu in PyTorch
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        x = F.relu(self.fc4(x))
        x = F.relu(self.fc5(x))
...
def getOptimizer(model, learning_rate, l2_rate):

  optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=l2_rate)
...
#def wasserstein(output, hammer, true_label, penalized_classes=[0,1,2,3]):
#
#    #this dictionary specifies which directions are considered for each signal
#
#    penalized_directions = {"0":[2],
#                            "1":[2],
#                            "2":[2],
#                            "3":[2]}
#
...
#    true_label = tf.argmax(true_label, axis=1)
#
#    for c in penalized_classes:
#
#        #only take events where true label is class c
#        mask = tf.equal(true_label, c)
#
#        #apply mask
#        output_filt = tf.boolean_mask(output, mask)
#        hammer_filt = tf.boolean_mask(hammer, mask)
#        hammer_central_w = hammer_filt[:, -1] #central weight is at last column
#   
#        
#
#        for i in penalized_directions[str(c)]:
#
#
#            hammer_dir = hammer_filt[:, i-1] #f.e. direction 2 is at entry 1 (2-1) in hammer_var
#
#            # weights
#            wx = hammer_central_w        # central weights
#            wy = hammer_dir              # direction-specific weights
#            
...
#
#            pdb.set_trace()
#            # compute weighted distributions over classes using the softmax output
#            p = tf.reduce_sum(output_filt * tf.reshape(wx, (-1,1)), axis=0)  # shape: (num_classes,)
#            q = tf.reduce_sum(output_filt * tf.reshape(wy, (-1,1)), axis=0)  # shape: (num_classes,)
#            
#            # normalize again to be proper probability distributions
#            p /= tf.reduce_sum(p)
#            q /= tf.reduce_sum(q)
#            
...
#            cost = tf.reduce_sum(tf.abs(cdf_p - cdf_q))
#
#            #cost = tf.reduce_sum(p * tf.math.log(p / (q + 1e-10)))
#
#            wsd += cost
#
#    return wsd
...
#def wasserstein_mlg(output, hammer, true_label, penalized_classes=[0,1,2,3]):
#
...
#      hammer_central_w = hammer_filt[:, -1] #central weight is at last column
#
#      #get the class prediction rather than softmax score
#      class_pred = tf.cast(tf.argmax(output_filt, axis=1), tf.float64) 
#      
#
#      for i in penalized_directions[str(c)]:
#
...
#          wy = hammer_dir              # direction-specific weights
#
#          #multiply with class prediction
#          wx = wx * class_pred
#          wy = wy * class_pred
#          
#          pdb.set_trace()
...
class Sample(object):
  ...
  def __init__(self, filename,tree, selection,signal):
    ...
    if self.signal in [0,1,2,3]:
      ...
      branches = kin_var + ['event'] + hammer_var #+ ["central_w"]  #event will be removed later!
...
class Trainer(object):
  ...
  def __init__(self, features, epochs, batch_size, learning_rate, l2_rate, lambda_penalty_scalar, lambda_penalty_vector, scaler_type, es_patience, do_reduce_lr, dirname, baseline_selection, nfolds, frac_sb, frac_sf):
  ...
  def createOutDir(self):
    ...
    f.write("l2 rate: "                   + str(self.l2_rate)             + "\n")
  ...
  def preprocessing(self, train_samples, test_samples):
    ...
    #lets redefine the central_w column with the normalized weight
    train["central_w"] = train["central_w"] / central_av_train  
    test ["central_w"] = test ["central_w"] / central_av_test
    ...
    for e in hammer_dir:
      ...
      #lets redefine the variational weights columns with the normalized weight
      train[e] =  train[e] /  var_av_train[f"average_{e}"]
      test [e] =  test [e] /  var_av_test [f"average_{e}"]

      #REMARK: the entries e7-e10 are nonsense for dsmu and dstau here! Hence:

      #replace negative weights with 1.0
      train[e] = train[e].mask(train[e] < 0, 1.0)
      test[e]  = test[e] .mask(test[e]  < 0, 1.0)
      #replace nan weights with 1.0
      train[e] = train[e].fillna(1.0)
      test[e]  = test[e] .fillna(1.0)


    #the total weight consists of sf weights (1 for mc) and hammer central weights (1 for data & hb)
    ...
    hammer_var_cols = hammer_var + ["event"]
  ...
  #  model.add(tf.keras.layers.Dense(128,  activation ='swish'  ,  kernel_regularizer=l2_rate))
  #  model.add(tf.keras.layers.Dense(256,  activation ='swish'  ,  kernel_regularizer=l2_rate))
  #  model.add(tf.keras.layers.Dense(256,  activation ='swish'  ,  kernel_regularizer=l2_rate))
  #  model.add(tf.keras.layers.Dense(128,  activation ='swish'  ,  kernel_regularizer=l2_rate))
  #  model.add(tf.keras.layers.Dense(128,  activation ='swish'  ,  ))#kernel_regularizer=l2_rate))
  ...
  def prepareInputs(self, xx_folds, y_folds, w_folds, h_folds):
    ...
    pdb.set_trace()
    for col in xx_folds[0].columns:
      plt.figure()
      plt.hist(pd.concat([ xx_folds[i][col] for i in range(self.nfolds)]))
      self.saveFig(plt, "scaled_" + col + "_distro.pdf")
  ...
  def train(self, xx_folds, y_folds, w_folds, h_folds, weight, class_label):
    ...
    for n in range(self.nfolds):
      ...
      # debug: use 2 classes only

      #xx_train[n] = xx_train[n][y_train[n]["is_signal"].isin([2,5])] 
      #w_train[n]  = w_train[n] [y_train[n]["is_signal"].isin([2,5])] 
      #h_train[n]  = h_train[n] [y_train[n]["is_signal"].isin([2,5])] 
      #y_train[n]  = y_train[n] [y_train[n]["is_signal"].isin([2,5])] 
      #y_train[n]['is_signal'] = y_train[n]['is_signal'].replace({2: 0, 5: 1})

      #xx_val[n]   = xx_val[n][y_val[n]["is_signal"].isin([2,5])] 
      #w_val[n]    = w_val[n] [y_val[n]["is_signal"].isin([2,5])] 
      #h_val[n]    = h_val[n] [y_val[n]["is_signal"].isin([2,5])] 
      #y_val[n]    = y_val[n] [y_val[n]["is_signal"].isin([2,5])] 
      #y_val[n]['is_signal'] = y_val[n]['is_signal'].replace({2: 0, 5: 1})

      #convert to torch 
      xx_train[n] = torch.tensor(xx_train[n].values, dtype=torch.float32) 
      xx_val[n]   = torch.tensor(xx_val  [n].values, dtype=torch.float32)

      #convert to torch (use long int), no on ehot enconding needed!
      y_train[n]  = torch.tensor(y_train[n]['is_signal'].values, dtype=torch.long)
      y_val  [n]  = torch.tensor(y_val  [n]['is_signal'].values, dtype=torch.long)

      # convert to torch 
      w_train[n] = torch.tensor(w_train[n]["total_w"].values, dtype=torch.float32) 
      w_val[n]   = torch.tensor(w_val  [n]["total_w"].values, dtype=torch.float32)

      h_train[n] = torch.tensor(h_train[n].values, dtype=torch.float32) 
      h_val[n]   = torch.tensor(h_val  [n].values, dtype=torch.float32)

      #clear model and redefine a new one for every fold! 

      model[n] = torchModel(dimIn=len(features), dimOut=6).cuda() 
      opt      = getOptimizer(model[n], self.learning_rate, self.l2_rate)
      ...
      # prepare splitting into batches
      tensorDataset_train = TensorDataset(xx_train[n], y_train[n], w_train[n])
      tensorDataset_val   = TensorDataset(xx_val[n],   y_val[n],   w_val[n]  )

      loader_train        = DataLoader(tensorDataset_train, batch_size=batch_size, shuffle=True , num_workers=8, prefetch_factor=2, pin_memory=True,)
      loader_val          = DataLoader(tensorDataset_val  , batch_size=batch_size, shuffle=False, num_workers=8, prefetch_factor=2, pin_memory=True,)

      # we want a crossentropy loss
      # important: turn off the averaging over batch (i.e. reduction == none)
      #            as we want to apply sample weights later (manually)

      class_w = []
      total_pop = 0
      for i in range(0,6):
        total_pop += (y_train[n]==i).sum().item()
      for i in range(0,6):
        class_w.append( total_pop / (y_train[n]==i).sum().item() )

      class_w = torch.tensor(class_w, dtype=torch.float32, device = "cuda")

      #normalize
      class_w =  class_w / class_w.sum() 
      
      #loss_model = torch.nn.CrossEntropyLoss(reduction='none')      
      loss_model = torch.nn.CrossEntropyLoss(weight = class_w.cuda())      
      ...
      #debugging - get single batch with:
      #train_iter = iter(loader_train)
      #batch_x, batch_y, batch_w = next(train_iter)
      ...
      #########################################################
      # Avoid any operation that requires GPU -> CPU transfer #
      # in the training loop.                                 #
      #########################################################

      #half precision but speedy gonzales
      scaler = torch.cuda.amp.GradScaler(init_scale=2**12)

      for epoch in range(epochs):

        #prepare the model - this does not train yet, we just enter the training mode
        model[n].train()
        ...
        #loop over batches
        for batch_x, batch_y, batch_w in loader_train:

          batch_x = batch_x.cuda(non_blocking=True) 
          batch_y = batch_y.cuda(non_blocking=True)
          batch_w = batch_w.cuda(non_blocking=True)

          print(batch_x.min(), batch_x.max(), batch_x.mean())


          # class_w is a tensor holding the weight per class (at the right index) [w0, w1, w2, ..., w5]
          # batch_y is a tensor holding the class prediction [1,3,4,2,2,5,....]
          # indexing with a long tensor -> for every event in batch_y, get the corresponding weight 
          weights_in_this_batch = class_w[batch_y] # same shape as batch_y
          batch_weight = weights_in_this_batch.sum() 

          # (re)set gradients to zero          
          opt.zero_grad()

          with torch.cuda.amp.autocast():
            # get outputs of batch x 
            outputs = model[n](batch_x)
            # get bare loss by applying crossentropy
            bare_loss = loss_model(outputs, batch_y)

          if not torch.isfinite(bare_loss):
            pdb.set_trace()

          #compute new gradients 
          #bare_loss.backward()
          scaler.scale(bare_loss).backward()
          ...
          #add to total loss (for one epoch) and weigh by batch weight
          total_loss += bare_loss.detach() * batch_weight

          #append batch weight to total weight
          total_weight += batch_weight

        pdb.set_trace()
        avg_train_loss = total_loss / total_weight

        history[n]["total_loss_train"].append(avg_train_loss) 

        #prepare the model for validation mode
        model[n].eval()
        total_loss = torch.tensor(0.0, device="cuda")
        #total_weight = torch.tensor(0.0, device="cuda")

        # we don't compute any gradients here, we just evaluate        
        with torch.no_grad():

          for batch_x, batch_y, batch_w in loader_val:
       
            batch_x = batch_x.cuda(non_blocking=True) 
            batch_y = batch_y.cuda(non_blocking=True)
            batch_w = batch_w.cuda(non_blocking=True)

            #get outputs on validation batches and calculate weighted loss
            outputs = model[n](batch_x)
            bare_loss = loss_model(outputs, batch_y)
            ...
            #predictions are argmax' of the softmax arrays   
            #preds = outputs.argmax(dim=1)
            #correct += ((preds == batch_y) * batch_w).sum().item()  
            #total += batch_w.sum().item()
   
        avg_val_loss = total_loss / total_weight 

        history[n]["total_loss_val"].append(avg_val_loss) 

        #acc_val = correct / total
        #history[n]["acc_val"].append(acc_val) 
 

        #print epoch summary
        print(f"Epoch {epoch+1}/{epochs} | Train Loss: {avg_train_loss} | Val Loss: {avg_val_loss}") # | Val Acc: {acc_val:.4f}")
  ...
  def plotMetric(self, history, history_key, title, ylabel, fold = None):
    ...
    if fold is None:
      ...
      for n in range(self.nfolds):
        ...
        #this is cuda type tensor, convert to cpu 
        toPlot = [loss.cpu().numpy() for loss in history[n][history_key]] 
  ...
  def plotPred(self, model, xx_train, y_train, xx_val, y_val, class_label, fold ):

    #get numpy version
    x_val_n = xx_val[n].to(device)
    y_val_n = y_val[n].cpu().numpy() # already true label for torch!
  
    #select only events where the true class is chan...
    x_chan    = xx_val[n][ y_val_n == class_label ]

    #...and predict their score for class sig! (data is already scaled!)
    with torch.no_grad():
      logits = model[n](x_chan)
      probs = torch.softmax(logits, dim=1)  # convert to probabilities

      # get the predictions as the argmax of the probs
    ...
    if fold is None:
      ...
      for chan in channels:
        ...
        for n in range(self.nfolds):
          ...
          #select only events where the true class is chan...
          x_chan    = xx_val[n][ y_val_n == chan ]
          #...and predict their score for class sig! (data is already scaled!)
          with torch.no_grad(): 
            logits = model[n](x_chan)
            probs = torch.softmax(logits, dim=1)  # convert to probabilities
            y_chan = probs[:, sig].cpu().numpy() 
      ...
      for chan in channels:
        ...
        #get numpy version
        xx_val_n = xx_val[n].to(device)
        y_val_n  = y_val[n].cpu().numpy()  # already true label for torch!
        
        #select only events where the true class is chan...
        x_chan    = xx_val_n[ y_val_n == chan ]
        #...and predict their score! (data is already scaled!)
        with torch.no_grad(): 
          logits = model[n](x_chan)
          probs = torch.softmax(logits, dim=1)  # convert to probabilities
      ...
      plt.xscale("log")
      self.saveFig(fig, f'score_' + str(sig) + f"_fold_{fold}_log" )
      plt.clf()
      plt.close()
  ...
  def plotKSTest(self, model, xx_train, y_train, xx_val, y_val,sig, fold = None):
    ...
    if fold is None:
      ...
      #get numpy version
      x_train_n = xx_train[n].to(device)
      y_train_n = y_train[n].cpu().numpy() # already true label for torch!

      x_val_n = xx_val[n].to(device)
      y_val_n = y_val[n].cpu().numpy() # already true label for torch!

      #select only events where the true class is sig...
      x_chan_val    = xx_val[n][ y_val_n == sig ]
      x_chan_train    = xx_train[n][ y_train_n == sig ]
      #...and predict their score for class sig! (data is already scaled!)
      with torch.no_grad(): 

        logits_train = model[n](x_chan_train.cuda())
        probs_train = torch.softmax(logits_train, dim=1)  # convert to probabilities
        y_chan_train = probs_train[:, sig].cpu().numpy() 

        logits_val = model[n](x_chan_val.cuda())
        probs_val = torch.softmax(logits_val, dim=1)  # convert to probabilities
        y_chan_val = probs_val[:, sig].cpu().numpy() 
  ...
  def process(self):
    ...
    # do the training
    #print('\n========> training...') 
    model, xx_train, y_train, h_train, xx_val, y_val, h_val = self.train(xx_folds, y_folds, w_folds, h_folds, weight, class_label)
    ...
    return model, xx_folds, y_folds, w_folds, h_folds, weight, class_label
...
if __name__ == '__main__':
  ...
  learning_rate = 0.005
  l2_rate = 0.005
  lambda_penalty_scalar = 0.#10.0
  lambda_penalty_vector = 10000.#10.0
  scaler_type = 'robust'
  es_patience = 50
  do_reduce_lr = False
  dirname = 'test'
  ...
  #history, model, xx_folds, y_folds, w_folds, h_folds, weight, class_label = trainer.process()
  model, xx_folds, y_folds, w_folds, h_folds, weight, class_label = trainer.process()
  #trainer.process()
