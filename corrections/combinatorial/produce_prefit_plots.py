...
def combSys(histos2, var, scale_hb, scale_pimu = None, scale_rest = None, logx = False, logy = False, region = None, blind = False):
  ...
  histos["Data"].Scale(1.0/histos["Data"].Integral())
  ...
  if logy:
    ...
    maxi = max([hComb.GetBinContent(hComb.GetMaximumBin()), hComb.GetBinContent(hComb.GetMaximumBin())])
    yAxisTitle = "events"
    hComb.GetYaxis().SetTitle(yAxisTitle)
  ...
  #append also last edge :)
  edges.append(ratio.GetBinLowEdge(i) + ratio.GetBinWidth(i)) 
  ...
  with open(f"{toSave}/comb_sys_weights_up_{var}_{region}.json","w") as f:
    json.dump(weights_up, f) 

  with open(f"{toSave}/comb_sys_weights_down_{var}_{region}.json","w") as f:
    json.dump(weights_down, f) 

  with open(f"{toSave}/comb_sys_edges_{var}_{region}.json","w") as f:
    json.dump(edges, f) 
...
def writeBinnedDatacard(histos, var, region, digits = 5, blind = False):
  ...
  if hammer_sys and not bs_tau_sys:
    ...
    temp = open("/work/pahwagne/RDsTools/fit/new_temps/datacardTemplateSystematics_binned_starVSnonstar.txt", "rt")
...
for i in range(num_bins+1):
  ...
  for var in vars_base:
    ...
    histosScaled        = combSys(histos_reshuffled[var],       var, scale_hb,   scale_pimu = scale_pimu,       scale_rest = scale_rest,        region = f"ch{i}", blind = False )
